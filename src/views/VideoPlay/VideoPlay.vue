...
export default {
  ...
  mounted() {
    ...
    if (getToken()) {
      ...
      // 放置用户名到评论内容中
      this.userRemark["userId"] = getTokenValue("userId")
    }
    this.init()
  },
  methods: {
    ...
    setData(value) { //将获取来的视频数据赋值给vue
      ...
      console.log("count", this.coinsCount)
      this.collectCount = parseInt(value.videoAbout[0]['userLikeCount'])
      console.log("count", this.collectCount)
    ...
    ifLike() {
      if (getToken()) {
        ...
        this.userMessage["userPraiseTime"] = new Date()
    ...
    ifCollect() {
      if (getToken()) {
        ...
        this.userMessage["userLikeTime"] = new Date()
        this.collect(this.userMessage, state)
        console.log("this.isCollect", state)
        clearVideo("videoPlay")
        if (state) {
    ...
    ifShare() {
      ...
      if (getToken()) {
        ...
        this.userMessage["userShareTime"] = new Date()
